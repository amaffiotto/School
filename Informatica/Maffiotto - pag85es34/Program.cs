...
internal class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        int target = LeggiNumero("inserire il valore da cercare:");
        int[,] matrice = new int[r, c];
        CaricaMatrice(matrice, 1, 10);
        int count = ContaValore(matrice, target);
        StampaMatrice(matrice);
        Console.WriteLine($"Il numero: {target} compare {count} volte");
    }

    private static int ContaValore(int[,] matrice, int target)
    {
        int count = 0;
        for (int i = 0; i < matrice.GetLength(0); i++)
        {
