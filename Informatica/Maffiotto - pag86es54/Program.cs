...
internal class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        if (AreElementsBelowDiagonalEqual(matrice))
        {
            Console.WriteLine("Gli elementi sotto la diagonale principale sono uguali");
        }
        else
        {
            Console.WriteLine("Gli elementi sotto la diagonale principale NON sono uguali");
        }
    }

    private static bool AreElementsBelowDiagonalEqual(int[,] matrice)
    {
        bool result = true;
        int firstValue = 0;
        bool firstTime = true;
            ...
            for (int i = 0; i < matrice.GetLength(0); i++)
            {
                for (int j = 0; j < matrice.GetLength(0); j++)
                {
                    if (j < i)
                    {
                        if (firstTime)
                        {
                            firstValue = matrice[i, j];
                            firstTime = false;
                        }
                        else
                        {
                            if (matrice[i, j] != firstValue)
                            {
                                result = false;
                            }
                        }
